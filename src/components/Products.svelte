...
<div class="text-center">
  <p class="bg-blue-400 text-white p-4 mx-auto w-48 inline-block">
    Sort by:
  ...
  <button
    on:click={sort}
    class="bg-blue-400 text-white p-4 mx-auto w-48 inline-block"
  >
    Order by: {direction}
  </button>
</div>
...
<ul class="container mx-auto grid grid-cols-4 gap-12 my-12">
  {#each productList as product}<ProductCard
      {product}
      on:message={notify}
    />{/each}
</ul>
